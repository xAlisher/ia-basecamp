...
Item {
    ...
    ColumnLayout {
        ...
        ColumnLayout {
            ...
            // Settings — ⚙ glyph in a framed box (receiver_ui's host-proven pattern).
            // NOT LogosIconButton + data-URI SVG: that renders blank in the ui-host
            // (MultiEffect/data-URI-SVG combo). A LogosText glyph is robust. Box height
            // matches the status badges; accent active-border + tint when open.
            Rectangle {
                implicitWidth: gwBadge.implicitHeight; implicitHeight: gwBadge.implicitHeight
                radius: Theme.spacing.radiusSmall
                color: cogMa.containsMouse ? root.bgSecondary : "transparent"
                border.width: 1
                border.color: root.settingsOpen ? root.accentOrange : root.borderColor
                LogosText {
                    anchors.centerIn: parent
                    text: "⚙"
